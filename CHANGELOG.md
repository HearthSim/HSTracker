...
- Fixed the Battlegrounds comp guide's "Show Example Lineups" button hugging its label in the bottom left of the Core Cards section. Like Hearthstone Deck Tracker, it now spans the width of the section with its label centered, and when it is disabled because the account has no Tier7 subscription, hovering it explains why.
- The Battlegrounds comp guide list's mode badge now matches Hearthstone Deck Tracker: "Tier7 Version" behind the Tier7 logo, or "Free Version" with a "?" that explains on hover what a subscription adds to this screen. It was a bare "Tier 7 Mode" or "Free" label with no explanation.
## Bob's Buddy
...
- Fixed HSTracker crashing when Bob's Buddy re-read a minion's granted Crab deathrattles ("Crab Riding") during a combat.
