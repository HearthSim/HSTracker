...
class Shudderwock: ICardWithRelatedCards {
    ...
    func shouldShowForOpponent(opponent: Player) -> Bool {
        ...
        return CardUtils.mayCardBeRelevant(card: card, gameType: game.currentGameType, format: game.currentFormatType, playerClass: opponent.originalClass) && getRelatedCards(player: opponent).count > 3
    }
