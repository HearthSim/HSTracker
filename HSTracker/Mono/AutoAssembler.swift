...
class AutoAssembler: MonoClassInitializer {
    ...
    static func initialize() {
        if ReplicatingMenace._class == nil {
            ReplicatingMenace._class = MonoHelper.loadClass(ns: "BobsBuddy.Minions.Mech", name: "AutoAssembler")
            ReplicatingMenace._deathrattle = MonoHelper.getMethod(AutoAssembler._class, "Deathrattle", 1)
        }
    }
    ...
    static func deathrattle(golden: Bool) -> MonoHandle {
        ...
        let result: UnsafeMutablePointer<MonoObject>? = params.withMemoryRebound(to: UnsafeMutableRawPointer?.self, capacity: 1, {
            return mono_runtime_invoke(AutoAssembler._deathrattle, nil, $0, nil)
        })
        a.deallocate()
        params.deallocate()
