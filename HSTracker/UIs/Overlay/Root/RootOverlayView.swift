...
struct RootOverlayView: View {
    ...
    var body: some View {
        ...
        GeometryReader { geometry in
            // A degenerate measurement - the zero-size frame the overlay window
            // briefly carries before Hearthstone's own frame is known, and again
            // whenever the accessibility read comes back empty - makes canvasWidth
            // zero, and every widget that converts HDT's 4:3-relative X into canvas
            // X divides by it. That division hands SwiftUI a NaN position and the
            // layout engine traps on it, so nothing is laid out until the canvas is
            // real. There is nothing to see at that size anyway.
            if RootOverlayView.isUsableCanvas(geometry.size) {
                canvas(geometry)
            }
        }
    ...
    /// Whether a measurement is something the canvas can actually be laid out on.
    /// Both dimensions have to be finite and positive: a zero width is what makes
    /// `SizeHelper.getScaledXPos` return NaN, and a zero height would divide the
    /// same way through `scale`.
    static func isUsableCanvas(_ size: CGSize) -> Bool {
