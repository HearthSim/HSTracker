...
class MercenaryAbilityView: NSImageView {
    ...
    func render() {
        ...
        // Kick the art download outside the drawing handler: the handler is
        // invoked lazily by AppKit and must not schedule another render.
        if let cardId = vm.cardId, ImageUtils.cachedArt(cardId: cardId) == nil {
            ImageUtils.art(for: cardId, completion: { [weak self] (img: NSImage?) in
                if img != nil {
        ...
        // The handler captures only `vm` (an immutable struct) and calls a
        // static helper, so it holds no reference back to the view that stores
        // the image.
        self.image = NSImage(size: NSSize(width: rect.width, height: 280), flipped: false, drawingHandler: { _ -> Bool in
            if vm.active {
            ...
            if let cardId = vm.cardId, let cardImage = ImageUtils.cachedArt(cardId: cardId) {
            ...
            if let cooldownText = vm.cooldownText {
                MercenaryAbilityView.drawText(text: cooldownText, rect: NSRect(x: 256-30-100, y: 256+9-85, width: 100, height: 100), color: NSColor.white, size: 100, stroke: -3)
            }

            MercenaryAbilityView.drawText(text: vm.speedText, rect: NSRect(x: 40, y: 10, width: 150, height: 150), color: vm.speedColor, size: 100, stroke: -3)
            if vm.cooldownShadingVisibility {
                MercenaryAbilityView.drawText(text: vm.speedText, rect: NSRect(x: 40, y: 10, width: 150, height: 150), color: cooldownShadingColor, size: 100, stroke: -3)
            }

            if vm.speedUncertainIndicatorVisibility {
                MercenaryAbilityView.drawText(text: "?", rect: NSRect(x: 160, y: 10, width: 120, height: 120), color: NSColor.white, size: 90, stroke: -2)
                if vm.cooldownShadingVisibility {
                    MercenaryAbilityView.drawText(text: "?", rect: NSRect(x: 160, y: 10, width: 120, height: 120), color: cooldownShadingColor, size: 90, stroke: -2)
                }
            }
    ...
    static func drawText(text: String, rect: NSRect, color: NSColor, size: CGFloat, stroke: Int) {
