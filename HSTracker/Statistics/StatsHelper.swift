...
class StatsHelper {
    ...
    static func getStatsUITableData(deck: Deck) -> [StatsTableRow] {
        ...
        for againstClass in ["all"] + StatsHelper.playerClassList {
            ...
            dataRow.confidenceInterval = getDeckConfidenceString(deck, againstClass: againstClass, confidence: 0.9)
            let stats = getDeckRecord(deck, againstClass: againstClass)
            let interval = binomialProportionCondifenceInterval(stats.wins, losses: stats.losses, confidence: 0.9)
    ...
    static func getDeckConfidenceString(deck: Deck, againstClass: String = "all", confidence: Double = 0.9) -> String {
        let stats = getDeckRecord(deck, againstClass: againstClass)
        let interval = binomialProportionCondifenceInterval(stats.wins, losses: stats.losses, confidence: confidence)
    ...
    static func binomialProportionCondifenceInterval(wins: Int, losses: Int, confidence: Double = 0.9) -> (lower: Double, upper: Double, mean: Double) {
